...
class Akategori extends CI_Controller {
		...
		public function update(){
			$this->akategori_m->update($this->input->post('idkategori_tmp'),$this->input->post('kategori'));
			redirect(base_url().'akategori');
		}

		public function delete()
		{	
			$this->akategori_m->delete($this->input->post("idkategori"));
			redirect(base_url().'akategori');
		}
		...
		public function export()
		{
			if($this->uri->segment(3) == "excel"){
				...
				header('Content-Disposition: attachment; filename="Daftar Kategori Jobstreet.xls"');
